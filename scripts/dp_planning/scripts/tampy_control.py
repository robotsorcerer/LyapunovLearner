...
def unordered_motions(data_file):
    start = time.time()
    home_pos = [0.0] * 7
    torobo = ToroboEnvironment(home_pos)
    ...
    while time.time() - start < 200:
        ...
        temp_positions[3] = 0.69 * math.sin(temp_positions[3]*i)
        temp_positions[4] = 0.05 * math.sin(temp_positions[4]*i)
        temp_positions[5] = 0.08 * math.sin(temp_positions[5]*i)
        temp_positions[6] = 1.61 * math.sin(temp_positions[6]*i)

        if i % 2 == 0:
            temp_positions[2] *= -1
            temp_positions[3] *= -1

        logger.debug("setting and saving positions and velocities")
        torobo.set_position(temp_positions)
...
if __name__ == '__main__':
    data_file = "unordered_motions.csv"
