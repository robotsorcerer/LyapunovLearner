...
class RealTimePlotter(object):
    ...
    def __init__(self, fig, gs, time_window=500, \
                labels=None, alphas=None, nbSPoint=3):
        self._fig = fig
        self._gs = gridspec.GridSpec(1, 1)
    ...
    def init(self, data_len):
        ...
        self._ax.legend(loc='upper right')#, bbox_to_anchor=(0, 1.15))
    ...
    def update(self, x, xT):
        ...
        for j in range(self._num_conds):
            print('x: ', x.shape)
            self._plots[j].set_data(x[0,j], x[1, j])
            # ,  'ko', \
