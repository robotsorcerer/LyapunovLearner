...
if (isfield(options, 'video') && options.video)
  if ~isfield(options, 'videoFName')
      options.videoFName = [datestr(now,'YYYYMMDD_hhmmss') '.mp4'];
  end

  vidObj = VideoWriter(options.videoFName,'MPEG-4');
  vidObj.Quality = 100;
  vidObj.FrameRate = 30;
  try
    vidObj.open;
  catch
    error('Cannot open the video object for writing')
  end
end
...
while true
    ...
    % If we're making a video, grab the frame
      % fprintf('b4 make video.')
    if isfield(options, 'video') && options.video
      % fprintf('Trying to make video.')
      current_frame = getframe(gcf); %gca does just the plot
      writeVideo(vidObj,current_frame);
    end
...
function options = check_options(varargin)
  ...
  if ~isfield(options, 'video')
    ...
    if ~isfield(options, 'videoFName') % we must make videos; how else are they gon' know?
      options.videoFName = 'imitation_2D_s.mp4';
    end
  end
...
switch mode
    case 'i' %initializing the figure
        if d==2
            sp.fig = figure('name','2D Simulation of the task','position',[653 400 560 420]);
            sp.axis = gca;
            hold on
